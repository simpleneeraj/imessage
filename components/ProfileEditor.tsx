      ...
      <DrawerPopup showBar variant="inset">
        ...
        <div className="flex flex-col gap-5 px-6 pb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {MOODS.map((m) => (
              <button
                ...
                type="button"
                onClick={() => setMood(m)}
                className="cursor-pointer rounded-full border border-border px-3 py-1 text-[13px] hover:bg-muted/40"
              >
                {m}
              </button>
            ))}
          ...
          <Button
            ...
            onClick={() => void save()}
            disabled={saving}
            className="h-11 cursor-pointer rounded-full bg-primary text-[16px] font-semibold text-white hover:bg-primary/90"
          >
            Save
          </Button>
        </div>
